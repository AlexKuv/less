    ...
    cardDetailChangeElems.forEach((btn, i) => {
      btn.addEventListener('click', () => {
        if(!btn.classList.contains('active')) {
          ...
          cardImageItemElem.alert = data[i].name;
  ...
  const accordion = () => {
    ...
    const closeAllDrops = (button, dropDown) => {
      characteristicsItemElems.forEach((elem) => {
        if(elem.children[0] !== button && elem.children[1] !== button) {
...
// рендер карточек
  const renderCrossSell = () => {
    ...
    const createCrossSaleItem = (good) => {
    ...
    const render = arr => {
      arr.forEach(item => {
        crossSellList.append(createCrossSaleItem(item));
      });
    };
